...
c.JupyterHub.db_url = "sqlite:////etc/jupyterhub/mutable_settings/jupyterhub.sqlite"

c.Authenticator.admin_users = {"admin"}
c.Authenticator.allowed_users = {"admin"}

c.PAMAuthenticator.create_system_users = True
c.PAMAuthenticator.add_user_cmd = ["/etc/jupyterhub/immutable_settings/add_user.sh"]

c.JupyterHub.hub_ip = "0.0.0.0"
c.JupyterHub.hub_connect_ip = "jupyterhub.pydevenv.internal"
...
class CustomDockerSpawner(DockerSpawner):
    async def start(self):
        ...
        self.extra_create_kwargs.update(
            {
                "user" : f"0:{gid}",
                "hostname" : f"jupyterlab-{self.user.name}.pydevenv.internal",
            }
        )
...
def pre_spawn_hook(spawner):
    work_dir = f"/home/{spawner.user.name}/work"

    if not os.path.isdir(work_dir):
        os.mkdir(work_dir)

    os.chown(work_dir,
             pwd.getpwnam(spawner.user.name).pw_uid,
             grp.getgrnam("users").gr_gid)
    os.chmod(work_dir, 0o775)

c.Spawner.pre_spawn_hook = pre_spawn_hook

c.DockerSpawner.image = "pydevenv_singleuser"

c.DockerSpawner.notebook_dir = "/pydevenv"
c.DockerSpawner.mounts = [
    {
        "type" : "bind",
        "source" : os.path.join(os.getenv("PYDEVENV_DIR"),
                                "volumes/mutable/home/{username}/work"),
        "target" : os.path.join(c.DockerSpawner.notebook_dir, "work"),
    },
    {
        "type" : "bind",
        "source" : os.path.join(os.getenv("PYDEVENV_DIR"),
                                "volumes/mutable/share"),
        "target" : os.path.join(c.DockerSpawner.notebook_dir, "share"),
    },
]
...
c.DockerSpawner.network_name = "pydevenv_net"
